...
contract('RewardToken and Staker', (accounts) => {
  ...
  before(async () => {
    ...
    RewardTokenContract = await RewardToken.new(tokens('1002000'))
    ...
    // 1K RWT Tokens transfered to each Staker
    await RewardTokenContract.transfer(accounts[0], tokens('1000'))
    await RewardTokenContract.transfer(accounts[1], tokens('1000'))
    await RewardTokenContract.transfer(accounts[2], tokens('1000'))
  ...
  describe('Contracts deployment', async () => {
    it('Staker contract successfully deployed', async () => {
  ...
  describe('Tokens Distribution', async () => {
    ...
    it('Stacker1 initially received 1 thousand tokens', async () => {
      const balance = await RewardTokenContract.balanceOf(accounts[1])
      assert.equal(balance.toString(), tokens('1000'))
    })
    it('Stacker2 initially received 1 thousand tokens', async () => {
      const balance = await RewardTokenContract.balanceOf(accounts[2])
      assert.equal(balance.toString(), tokens('1000'))
    })
  ...
  describe('*** Only one Investor depositing in staker contract', async () => {
    it('Staker1 deposits 100 RWT tokens successfully', async () => {
      RewardTokenContract = await RewardToken.new(tokens('1002000'))
      StakerContract = await Staker.new(RewardToken.address)

      let depositAmount = web3.utils.toWei("10");
      //await StakerContract.deposit( depositAmount, { from: accounts[0] })
      await RewardTokenContract.approve(StakerContract.address, depositAmount, { from: accounts[1] });
    ...
    /*  await RewardTokenContract.approve(StakerContract.address, depositAmount, { from: accounts[1] });
      ...
      //const balance = await RewardTokenContract.balanceOf(accounts[1])
      //assert.equal(balance, tokens('900'))*/
    })

   /* it('Staker1 Initial rewards is 0 RWT Tokens', async () => {
